 ...
 * sensor. It allows the reading of raw and processed color values. The sensor
 * has a tri-color LED and this can be set to output red/green/blue or off. It
 * also has a full mode in which four samples are read (off/red/green/blue) very
 * quickly. These samples can then be combined using the calibration data
 * provided by the device to determine the "LEGO" color currently being viewed.
 * 
 * @author andy
 */
public class ColorSensorOLD implements LampLightDetector, ColorDetector,
		SensorConstants {

	protected static int[] colorMap = { -1, Color.BLACK, Color.BLUE,
	...
	private int lampColor = Color.WHITE;

	/**
	 * Extended color class, that includes the background reading at the time
	 * that the other readings were made.
	 */
	 ...
	 * 
	 * @param port
	 *            Port to use for the sensor.
	 */
	public ColorSensorOLD(SensorPort port) {
	 ...
	 *            Port to use for the sensor.
	 * @param color
	 *            The floodlight color.
	 */
	public ColorSensorOLD(SensorPort port, int color) {
		this.port = port;
	 ...
	 * @return Color data
	 */
	public ColorSensorOLD.Color getColor() {
		if (type != TYPE_COLORFULL) {
			...
			port.readValues(all_vals);
		return new Color(all_vals[RED_INDEX], all_vals[GREEN_INDEX],
				all_vals[BLUE_INDEX], all_vals[BLANK_INDEX], this.getColorID());
	}
	 ...
	 */
	public ColorSensorOLD.Color getRawColor() {
		if (type != TYPE_COLORFULL) {
			int temp_type = type;
			setType(TYPE_COLORFULL);
			port.readRawValues(all_vals);
			...
			port.readRawValues(all_vals);
		return new Color(all_vals[RED_INDEX], all_vals[GREEN_INDEX],
				all_vals[BLUE_INDEX], all_vals[BLANK_INDEX], Color.NONE);
	}
	...
	public boolean setFloodlight(int color) {
		...
		case Color.RED:
			lampColor = color;
			setType(ColorSensorOLD.TYPE_COLORRED);
			break;
		case Color.BLUE:
			lampColor = color;
			setType(ColorSensorOLD.TYPE_COLORBLUE);
			break;
		case Color.GREEN:
			lampColor = color;
			setType(ColorSensorOLD.TYPE_COLORGREEN);
			break;
		case Color.NONE:
			lampColor = color;
			setType(ColorSensorOLD.TYPE_COLORNONE);
			break;
		case Color.WHITE:
			lampColor = color;
			setType(ColorSensorOLD.TYPE_COLORFULL);
			break;
		default:
			return false;
		}
	...
	// TODO: Since this calibrate code (and other code) is the same for every
	// sensor, perhaps we should consider abstract classes to inherit shared
	// code from
	/**
	 * call this method when the light sensor is reading the low value - used by
	 * readValue
	 **/
	...
	/**
	 * call this method when the light sensor is reading the high value - used
	 * by readValue
	 */
	...
	/**
	 * Read the current color and return an enumeration constant. This is
	 * usually only accurate at a distance of about 1 cm.
	 * 
